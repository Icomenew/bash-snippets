...
#		rm -f $caseLog &>/dev/null
		...
		#yuvs=`grep $testId $scenarioFile | cut -d, -f3 | `
		...
		for yuv in $yuvs;do
			[ ! -f "$yuvDir/$yuv" ] && echo "Copy content_vp8/$yuv to $yuvDir" && cp -f content_vp8/$yuv $yuvDir/$yuv
		done
		...
		freeSpace=$((`df -l | awk '/home/{print $4}'`/(1024*1024)))
		[ "$freeSpace" -lt 50 ] && rm -f "$yuvDir/$yuv" 
